...
public class AutoSuggestor {
    ...
    public AutoSuggestor(JTextComponent textComp, Window mainWindow, ArrayList<String> words, Color popUpBackground, Color textColor, Color suggestionFocusedColor, float opacity, int Wordlimit) {
    ...
    public boolean wordTyped(String typedWord) {
        ...
        for (String word : dictionary) {//get words in the dictionary which we added
            ...
            if (l >= WordLimit){ break; }
            
            boolean fullymatches = true;
            //for (int i = 0; i < typedWord.length(); i++) {//each string in the word

                //if (!typedWord.toLowerCase().startsWith(String.valueOf(word.toLowerCase().charAt(i%word.length())), i)) {//check for match
                if (!word.toLowerCase().contains(typedWord.toLowerCase())) {fullymatches = false;} 
